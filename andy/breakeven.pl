...
while ( $set < 1000_000 ) {
  ...
  while ( $insert < 1000_000 ) {
    ...
    unless ( $done{$key}++ ) {
      ...
      $bm{base}
       = time_insert( $set, $insert, 'Set::IntSpan::Fast', 'add' );
      $bm{merge} = time_insert( $set, $insert, 'Set::IntSpan::Fast::XS',
        '_add_merge' );
      $bm{splice}
       = time_insert( $set, $insert, 'Set::IntSpan::Fast::XS',
        '_add_splice' );
      $bm{auto}
       = time_insert( $set, $insert, 'Set::IntSpan::Fast::XS', 'add' );
      ...
      printf(
        ...
        int( $insert ),
        join( ', ',
          map { sprintf( "%s (%6.2f)", $_, $bm{$_} / $best ) } @order ),
        $auto > $best
        ? sprintf( " --- auto %6.2f times slower", $auto / $best )
        : ''
      );
    }
    $insert *= 2;
